...
func TestSelectProvider(t *testing.T) {
	...
	t.Run("avito с каталогом страниц — ок", func(t *testing.T) {
		p, err := selectProvider("avito", &config.Config{}, "providers/avito/testdata")
		if err != nil || p.Name() != "avito" {
			t.Fatalf("avito: p=%v err=%v", p, err)
		}
	})

	// Та же логика fail-fast, что и у vk: без --input провайдер прочитал бы
	// пустоту и затёр прошлый сбор «успешным» пустым objects.json.
	t.Run("avito без --input — ошибка на старте", func(t *testing.T) {
		if _, err := selectProvider("avito", &config.Config{}, ""); err == nil {
			t.Fatal("ожидал ошибку про --input, получил nil")
		}
	})
