...
//
// [Go для изучения] Зачем мьютекс, если в JS он не нужен? Node крутит JS в одном
// потоке, а Go-сервер обслуживает каждый HTTP-запрос в своей ГОРУТИНЕ — хендлеры
// читают bySlug параллельно, а reload его перезаписывает. Без блокировки это
// data race (го ловит их флагом `go test -race`). RWMutex мягче обычного Mutex:
// RLock пускает много читателей ОДНОВРЕМЕННО, Lock (писатель) — эксклюзивен.
...
//
// [Go для изучения] `defer r.mu.RUnlock()` сразу после RLock — железная идиома:
// разблокировка гарантирована на любом пути выхода. Возврат (значение, ok) —
// «comma ok»: чтение из map отдаёт вторым значением, был ли ключ (undefined-а
// в Go нет — отсутствующий ключ даёт нулевое значение типа + ok=false).
