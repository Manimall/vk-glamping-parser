...
var extraRules = []amenityRule{
	...
	{[]string{"доп место", "дополнительное место", "доп. место"}, "Дополнительное место", ""},
	{[]string{"животн", "питом", "собак"}, "Размещение с животными", ""},
	{[]string{"доставк"}, "Доставка еды", ""},
	{[]string{"трансфер"}, "Трансфер", ""},
}

// Регэкспы для фактов. (?i) — без учёта регистра. Достаём вместимость, если она
// явно написана («до 4 чел», «максимум 4 человека», «для 2 человек»).
var (
...
func buildFacts(in Listing) []Fact {
	...
	facts := make([]Fact, 0)
	if m := reCapacity.FindStringSubmatch(text); m != nil {
...
func buildRules(text string) []string {
	keywords := []string{"заезд", "выезд", "заселен", "правил", "животн", "питом", "курен", "залог", "депозит"}
	rules := make([]string, 0)
