...
func TestBuildSEO_FromAbout(t *testing.T) {
	...
	d := seo.Description
	// Питч — ОДНО целое предложение из описания, с заглавной буквы. Раньше
	// строки склеивались через «. » и первая опускалась в нижний регистр:
	// склейка ломала длину (лимит проверялся только со второй строки), а
	// понижение регистра калечило имена собственные («Сабадури» → «сабадури»).
	if !strings.HasPrefix(d, "AFRAME — Уединённый и комфортный отдых") {
		t.Errorf("ожидал целое предложение с заглавной: %q", d)
	}
	if strings.Contains(d, "Купель Фурако") {
		t.Errorf("питч — одно предложение, а не склейка строк: %q", d)
	}
	if n := len([]rune(d)); n > seoDescTotalRunes {
		t.Errorf("описание длиннее лимита (%d > %d): %q", n, seoDescTotalRunes, d)
	}
