...
//
// [Go для изучения] `for range` по строке идёт по РУНАМ (Unicode code points),
// а не по байтам: «Д» — это 2 байта UTF-8, но одна руна. Аналог в JS —
// [...str] против str[i]. strings.Builder копит результат без пересоздания
// строки на каждом шаге (строки в Go неизменяемы, как и в JS: s += в цикле
// плодил бы аллокации).
