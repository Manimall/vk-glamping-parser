...
//
// Граница слова и делает функцию безопасной: настоящие латинские слова из
// описаний («A-frame», «SPA», «WhatsApp») кириллицы не содержат и остаются
// нетронутыми. Слепая замена по таблице превратила бы «A-frame» в «А-frame» —
// то есть сломала бы термин, вокруг которого построен весь каталог.
//
// ИЗВЕСТНОЕ ОГРАНИЧЕНИЕ: слитное смешанное слово («SPAкомплекс») уедет в
// кириллицу целиком. Более тонкие правила пробовали и отвергли — они разбирают
// «SPAкомплекс», но ломают «coздали», где подмены идут цепочкой подряд, а это
// реальные данные из сохранённых объявлений. Выбран разбор реальных подмен
// ценой редкого слитного англицизма: в описаниях он почти всегда либо через
// дефис («SPA-зона» — дефис слово разрывает, латиница цела), либо по-русски.
// Слаг при этом не страдает в любом случае: транслит вернёт ту же латиницу.
