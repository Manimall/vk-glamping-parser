...
#
# Списка «seo» здесь НЕТ намеренно, хотя провайдер это поле и не читает: внутри
# seo.imageData лежит запакованная карточка для соцсетей, а в ней — имя
# продавца («Оксана», «АлексейНина»). Глазами в base64 этого не видно, и первая
# версия фикстур утащила бы имена частных лиц в публичную историю git, откуда
# их убрать можно только переписыванием истории.
#
# Правило на будущее: добавлять поле сюда можно, только если провайдер его
# читает. Всё остальное — чужие данные, которым в репозитории не место.
ITEM_FIELDS = (
