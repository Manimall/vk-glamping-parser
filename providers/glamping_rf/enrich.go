...
// Дефолты MVP (см. applyDefaults). Только то, что в отрасли действительно
// стандартно: заезд после обеда, выезд до полудня — так почти везде, и ошибка
// здесь стоит гостю часа, а не поездки.
//
// Вместимости в этом списке БОЛЬШЕ НЕТ. Она подставлялась как «до 4» и была
// враньём: регулярка по вёрстке почти не совпадала (между словом
// «Вместимость» и числом стоят HTML-теги), и дефолт получал почти весь
// каталог — при реальном разбросе от двух гостей до двадцати семи. Гость видел
// «до 4», ехал вчетвером, а его ждала одна двуспальная кровать.
//
// Теперь «до 4» стоит у 29 объектов из 309 — потому что у них столько и есть.
//
// Теперь вместимость берётся из характеристик домиков (detail_specs.go), а
// если источник молчит — поля просто нет. Честное «неизвестно» лучше
...
func mergeDetail(obj *contract.Object, d *detailData) {
	...
	// Формы жилья из названий домиков ДОПОЛНЯЮТ теги списка, а не заменяют их.
	// У объекта с куполами и барнхаусами тег источника один — «Эко-дом», и он
	// отбрасывается как свалка; настоящие формы приходят только отсюда.
	obj.HouseTypes = mergeHouseTypes(obj.HouseTypes, d.HouseTypes)
	// Вместимость числом — для фильтра «нас шестеро». Строкой «до 6» она же
	// лежит в фактах карточки: одно значение, две формы для разных задач.
	if d.Guests > 0 {
		obj.GuestsMax = d.Guests
	}
...
func applyDefaults(obj *contract.Object) {
	...
	// Вместимость не выдумываем: нет данных — нет факта. См. комментарий к
	// дефолтам выше.
	if len(cp.Rules) == 0 {
