...
// «в лесу» только у 28 из 309 — там «Лесное озеро» чаще название, чем водоём.
...
// Одна дорога не должна давать два значения фильтра: источник пишет
// «М7 (Направление: Москва)» и «М7 (Направление: Казань)» — это М7 в обе
// стороны, а «Ново - рязанское» отличается от «Ново-рязанское» только тем, как
// поставлены пробелы. Ровно та же болезнь, что была у форм жилья.
func TestCanonHighway(t *testing.T) {
	cases := map[string]string{
		"М7 (Направление: Москва)":             "М7",
		"М7 (Направление: Казань)":             "М7",
		"Р158 (Направление: Арзамас, Саранск)": "Р158",
		"Ново - рязанское":                     "Ново-рязанское",
		"Симферопольское":                      "Симферопольское",
		"  Дмитровское  ":                      "Дмитровское",
		"":                                     "",
	}
	for in, want := range cases {
		if got := canonHighway(in); got != want {
			t.Errorf("canonHighway(%q) = %q, ожидали %q", in, got, want)
		}
	}
}

// Число километров без точки отсчёта несравнимо: источник меряет то от МКАД,
// то от города, и «70 км» у подмосковного объекта и у нижегородского означают
// разное. Поле обязано ехать вместе с числом.
func TestDistanceFromCarriesUnit(t *testing.T) {
	it := apiItem{City: apiCity{City: "Москва", Distance: "70", Unit: "км от МКАД"}}
	got := toObject(it)
	if got.DistanceKm != 70 {
		t.Errorf("DistanceKm = %d, ожидали 70", got.DistanceKm)
	}
	if got.DistanceFrom != "км от МКАД" {
		t.Errorf("DistanceFrom = %q — точка отсчёта потерялась", got.DistanceFrom)
	}
}
